...
import { ActiveSessionScribeProvider } from "@/components/patient-sessions/active-session-scribe-provider";
	...
	<Sidebar collapsible='icon' purpose='navigation'>
		...
		<SidebarContent>
			<div className='flex flex-col gap-1 px-3 py-2'>
				{["home", "patients", "notes", "mail", "calendar"].map((item) => (
		...
		<SidebarFooter>
			<div className='flex flex-col gap-1'>
				{["template", "settings", "logout"].map((item) => (
					<div key={item} className='flex h-8 items-center gap-2 rounded-md px-2'>
						<div className='size-4 animate-pulse rounded bg-sidebar-accent' />
					</div>
				))}
		...
		<div className='min-h-0 flex-1 overflow-y-auto'>
			<div className='w-full px-4 md:px-8'>
				...
				<div className='grid gap-4 pb-10 md:grid-cols-2'>
					{["patients", "notes", "mail", "calendar"].map((item) => (
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
	...
	return (
		<HydrateClient>
			<ActiveSessionScribeProvider>
				<DashboardActiveSessionShell>
