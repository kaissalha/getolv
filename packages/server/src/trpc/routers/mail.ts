...
export const createMailRouter = ({ createTRPCRouter, organizationProcedure }: getolvRouterFactoryOptions) =>
	createTRPCRouter({
		...
		getPatientThread: organizationProcedure
			.input(
				z.object({
			...
			.query(async ({ input, ctx }) =>
				getPatientMailThread({ ...input, organizationId: ctx.activeOrganizationId })
			),
