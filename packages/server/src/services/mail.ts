...
import { buildMailSearchRewritePrompt, mailSearchRewriteSchema } from "@getolv/ai/prompts";
import {
	createGmailDriver,
	type GmailDriver,
...
// ============================================================================
// Get Patient Mail Thread (aggregated from all conversations with patient)
// ============================================================================

export const getPatientMailThread = async ({
	organizationId,
	patientEmail,
	maxResults = 50,
}: {
	...
	const connection = await getMailConnection({ organizationId });
	...
	// Build query for all emails to/from the patient
	const query = `from:${patientEmail} OR to:${patientEmail}`;

	try {
		// List all threads matching the query
		...
		// Fetch full content for each thread and collect all messages
		const allMessages: ParsedMessage[] = [];
		const labelSet = new Set<string>();
		let hasUnread = false;

		// Fetch threads in parallel (limit concurrency to avoid rate limits)
		const threadResults = await Promise.all(
			threads.map(async (threadPreview) => {
				try {
					return await driver.getThread(threadPreview.id);
				} catch (error) {
					logger.error({
						error,
						message: "Failed to fetch patient mail thread",
						metadata: {
		...
		// Dedupe messages by id (in case same message appears in multiple threads)
		...
		// Sort messages by receivedOn date (oldest first for thread view)
		const sortedMessages = Array.from(messageMap.values()).sort(
			(a, b) => new Date(a.receivedOn).getTime() - new Date(b.receivedOn).getTime()
		);

		// Find the latest non-draft message
		const latest = sortedMessages.findLast((m) => !m.isDraft);
		const labels = await getSerializedMailLabels({ driver });

		return {
			classificationLabel: latest
				? getMailClassificationLabel({
			...
			latest,
			hasUnread,
			totalReplies: sortedMessages.filter((m) => !m.isDraft).length,
			labels: Array.from(labelSet).map((id) => ({ id, name: id })),
		};
	} catch (error) {
		logger.error({
			error,
			message: "Failed to build patient mail thread",
			metadata: {
				organizationId,
				patientEmail,
				maxResults,
