 ...
 */
type SidebarPurpose = "navigation" | "chat" | "patient-details";
...
export const sidebarAtoms = {
	...
	"patient-details": atomWithStorage("sidebar:patient-details:state", true, undefined, {
		getOnInit: true,
	}),
} as const;
 ...
 * - Chat/Patient-details: appears on the end side (right in LTR, left in RTL)
 */
const getSideFromPurpose = (purpose: SidebarPurpose, dir: "ltr" | "rtl" = "ltr"): Side => {
	if (purpose === "navigation") {
		return dir === "ltr" ? "left" : "right";
	}
	// chat, patient-details - all appear on the end side
...
const useSidebar = (purpose?: SidebarPurpose) => {
	const currentPurpose = useContext(CurrentSidebarPurposeContext);
	...
	const context = useContext(getSidebarContext(resolvedPurpose));
