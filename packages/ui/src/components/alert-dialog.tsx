...
const AlertDialogBackdrop = ({ className, ...props }: AlertDialogPrimitive.Backdrop.Props) => {
	return (
		<AlertDialogPrimitive.Backdrop
			...
			className={cn(
				"fixed inset-0 z-50 bg-foreground/32 backdrop-blur-sm transition-all duration-200 ease-out data-ending-style:opacity-0 data-starting-style:opacity-0",
				className
			)}
			{...props}
		/>
				...
				<div className='flex h-dvh flex-col items-center overflow-hidden pt-6 max-sm:before:flex-1 sm:overflow-y-auto sm:p-4 sm:before:basis-[20vh] sm:after:flex-1'>
					<AlertDialogPrimitive.Popup
						...
						className={cn(
							...
							"relative before:pointer-events-none before:absolute before:inset-0 before:shadow-lg max-sm:before:hidden sm:before:rounded-[calc(var(--radius-xl)-1px)] sm:data-nested:data-ending-style:translate-y-8 sm:data-nested:data-starting-style:translate-y-8 dark:shadow-lg dark:shadow-black/24 dark:before:shadow-[0_-1px_--theme(--color-white/8%)]",
							className
						)}
						{...props}
					/>
