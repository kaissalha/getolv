...
- Keep all AI prompts, output schemas used directly by prompts, and prompt tests in `packages/ai`; application and server code should import them from `@getolv/ai/prompts` instead of defining prompt strings inline.
...
- **packages/spoonacular**: Vendored OpenAPI client for the Spoonacular food and nutrition API (`@getolv/spoonacular`)
